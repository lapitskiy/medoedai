...
def run_multiprocessing_rolling_window(coin, period, date_df, window_size, threshold, numeric, config):
        ...
        for current_period in period:
            ...
            for current_window in window_size:
                for current_threshold in threshold:
                    # Запускаем процесс
                    futures.append(executor.submit(process_data, df.copy(), current_window, current_threshold, current_period, numeric, config))
        # Обработка завершенных задач
        for future in as_completed(futures):
            x_path, y_path = future.result()

def process_data(df, current_window, current_threshold, current_period, numeric, config):
    ...
    x_path, y_path, num_samples = create_rolling_windows(df, df_scaled, current_threshold, current_window, numeric, config)
    try:
...
def create_rolling_windows(df, df_scaled, current_threshold, input_window, numeric, config): # work BTC and TON and VOLUME
    ...
    if not os.path.exists('temp'):
        os.makedirs('temp')
    x_mmap = np.memmap(f'temp/{config.ii_path}/mmap/{uuid_mmap}_x.dat', dtype=np.float32, mode='w+', shape=(num_samples, input_window, num_features))
    y_mmap = np.memmap(f'temp/{config.ii_path}/mmap/{uuid_mmap}_y.dat', dtype=np.int8, mode='w+', shape=(num_samples, output_window))
    ...
    y_mmap._mmap.close()
    del x_mmap, y_mmap
    return f'temp/{config.ii_path}/mmap/{uuid_mmap}_x.dat', f'temp/{config.ii_path}/mmap/{uuid_mmap}_y.dat', num_samples
